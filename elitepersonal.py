...
def target():
    ...
    jalan('%s╠══[%s•%s] %s Locale: %s'%(O,P,O,P,lo))
    print('%s║'%(O))
    input('%s╚══[ %sReturn %s]%s'%(O,P,O,P))
    menu()
def teman_target():
...
def start_method():
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sMethod Api [Fast]'%(O,P,O,P))
    print('%s╠══[%s2%s] %sMethod Mbasic [Slow But Best]'%(O,P,O,P))
    print('%s╠══[%s3%s] %sMethod Free FB [Super Slow]'%(O,P,O,P))
def start_methodezz():
    print('%s║'%(O))
...
def started():
    ...
    print('%s╠══[%s•%s] %sMinimize Termux And Check Every 10 Minutes Later.\n'%(O,P,O,P))
    print('%s╠══[%s•%s] %sAirplane Mode Use Every 10 Mint.\n'%(O,P,O,P))
    print('\033[1;97m__________________________________________________________')
def folder():
    try:os.mkdir("CP")
    except:pass
    try:os.mkdir("OK")
